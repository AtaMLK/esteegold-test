...
create table if not exists public.commerce_orders (
  ...
  status text not null default 'pending_payment' check (status in ('pending_payment','paid','processing','shipped','delivered','canceled','payment_failed')),
  ...
  shipping_terms_accepted_at timestamptz,
...
create table if not exists public.commerce_order_items (
  ...
  product_line text not null check (product_line in ('EsteeGold','EsteeBags')),
  product_name_snapshot text not null,
  category_snapshot text,
  options_snapshot jsonb not null default '{}'::jsonb,
  quantity integer not null check (quantity > 0),
  unit_list_price numeric(12,2) not null check (unit_list_price >= 0),
  unit_discount numeric(12,2) not null default 0 check (unit_discount >= 0),
  unit_final_price numeric(12,2) not null check (unit_final_price >= 0),
...
-- No public write policies: checkout mutations use the server-side service-role client.
-- Add authenticated customer/admin read policies later according to the final auth model.
...
begin
  ...
  if v_order.payment_status = 'success' and v_order.status in ('paid','processing','shipped','delivered') then
