...
begin
  ...
  if p_new_status = v_order.status then
    return jsonb_build_object('success', true, 'unchanged', true, 'status', v_order.status);
  end if;
...
-- Refund inventory exactly once after the provider confirms the refund.
...
begin
  ...
  where payment_id = v_refund.payment_id and status = 'success' and id <> v_refund.id;
  ...
  for v_item in select * from commerce_order_items where order_id = v_order.id order by id for update loop
    ...
    if v_inventory.reserved_quantity >= v_item.quantity then
      ...
      insert into commerce_inventory_ledger(product_id, order_id, quantity_delta, available_delta, reserved_delta, reason)
      values(v_item.product_id, v_order.id, v_item.quantity, v_item.quantity, -v_item.quantity, 'refund_release');
