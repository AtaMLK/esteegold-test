...
export default function LoginForm() {
  ...
  useEffect(() => {
    ...
    gsap.set(loginPanelRef.current, { autoAlpha: nextMode === "login" ? 1 : 0 });
    gsap.set(registerPanelRef.current, { autoAlpha: nextMode === "register" ? 1 : 0 });
  }, [searchParams]);
  ...
  function flip(nextMode) {
    ...
    tl.to(outgoing, { autoAlpha: 0, duration: 0.22 }, 0)
      .fromTo(incoming,
        { autoAlpha: 0, x: showingRegister ? -22 : 22 },
      ...
      .to(side, {
        ...
        rotationX: showingRegister ? -1.8 : 0,
        rotationZ: showingRegister ? -0.8 : 0,
        skewY: showingRegister ? -1.2 : 0,
        borderRadius: showingRegister ? "0 3% 2% 0" : "0 0 0 0",
        boxShadow: showingRegister ? "24px 14px 50px rgba(0,0,0,.24)" : "0 0 0 rgba(0,0,0,0)",
        duration: 1.28,
        ease: "power4.inOut",
      }, 0)
      .to(side, {
        rotationY: showingRegister ? -185 : 5,
        rotationX: showingRegister ? 1.2 : -1.2,
        rotationZ: showingRegister ? 0.4 : 0.4,
        skewY: showingRegister ? 0.6 : 0,
        duration: 0.34,
        ease: "power2.out",
      }, 0.58)
      .to(side, {
  ...
  const renderPanel = (register) => (
    <section className={"auth-card auth-form-panel " + (register ? "register-panel" : "login-panel")} ref={register ? registerPanelRef : loginPanelRef} aria-hidden={register !== isRegister}>
      <div className="auth-heading"><p className="auth-kicker">{register ? "WELCOME IN" : "WELCOME BACK"}</p><h1>{register ? "Sign up." : "Sign in."}</h1><p className="auth-copy">{register ? "Create your account. No passport or ID is needed." : "Use your customer account or your authorised admin account."}</p></div>
      <form onSubmit={register ? handleRegister : handleLogin} className="auth-form">
        {register && <label><span>Name</span><input value={name} onChange={(e) => setName(e.target.value)} autoComplete="name" placeholder="Your name" required /></label>}
        ...
        <label><span>Password</span><div className="auth-password"><input value={password} onChange={(e) => setPassword(e.target.value)} type={showPassword ? "text" : "password"} autoComplete={register ? "new-password" : "current-password"} placeholder="Your password" minLength={6} required /><button type="button" onClick={() => setShowPassword((v) => !v)} aria-label={showPassword ? "Hide password" : "Show password"}>{showPassword ? <EyeOff size={16}/> : <Eye size={16}/>}</button></div></label>
        {!register && <div className="auth-forgot"><Link href="/auth/forgot-password">Forgot password?</Link></div>}
        {error && <p className="auth-error" role="alert">{error}</p>}
        {message && <p className="auth-success" role="status">{message}</p>}
        <button className="auth-primary" type="submit" disabled={loading}>{loading ? (register ? "Creating account…" : "Signing in…") : (register ? "Create account" : "Sign in")}<ArrowUpRight size={16}/></button>
      </form>
      ...
      <div className="auth-foot"><button type="button" className="auth-switch" onClick={() => flip(register ? "login" : "register")}>{register ? "Already have an account? Sign in" : "New here? Create an account"}</button>{!register && destination === "/admin" && <span>Admin access is enforced server-side by the authorised admin email.</span>}</div>
    </section>
  );
