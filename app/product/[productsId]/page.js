...
export default function ProductId({ params }) {
  ...
  function addToCart() {
    if (!product) return;
    addItem({ ...product, price: finalPrice(product), currency: product.regional_price?.currency || "EUR" }, quantity);
    setAdded(true);
  ...
  const regional = product.regional_price || { currency: "EUR", amount: Number(product.price || 0) };
  ...
  const symbol = currencySymbol(regional.currency);
    ...
    <main className="min-h-screen bg-[var(--paper)] px-5 pb-24 pt-28 md:px-10 md:pt-32">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-8 grid gap-10 lg:grid-cols-[1.15fr_0.85fr] lg:gap-20">
          ...
          <div className="flex flex-col justify-center py-5">
            ...
            <div className="mt-8 border-y border-black/10 py-6">
              {discounted && <p className="text-sm text-black/35 line-through">{symbol}{regional.amount.toFixed(2)}</p>}
              <p className="text-3xl tracking-[-0.03em]">{symbol}{price.toFixed(2)}</p>
              {discounted && <p className="mt-2 text-[9px] uppercase tracking-[0.2em]">{product.discount_percent}% house offer</p>}
              <p className="mt-2 text-[9px] uppercase tracking-[0.2em] text-black/40">{regional.currency} pricing · taxes and shipping calculated at checkout</p>
            </div>
        ...
        <section className="mt-20 grid gap-10 border-t border-black/10 pt-10 md:mt-32 md:grid-cols-[0.35fr_1fr] md:gap-10">
          ...
          <div><p className="max-w-3xl font-serif text-[clamp(2.4rem,4.5vw,5rem)] leading-[0.9] tracking-[-0.05em]">Made slowly. Kept for a reason.</p><p className="mt-8 max-w-2xl text-base leading-8 text-black/60">{story}</p></div>
        </section>
