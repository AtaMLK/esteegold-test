...
export default function ProductId({ params }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (cancelled) return;
        setProduct(found); setOptions({ material: found.material_options?.[0] || "", size: found.size_options?.[0] || "", stone: found.stone_options?.[0]?.name || found.stone_options?.[0] || "" });
        try {
  ...
  function addToCart() { if (!product) return; if (product.size_type !== "none" && !options.size) { setError("Please choose a size."); return; } if (product.stone_required && !options.stone) { setError("Please choose a stone."); return; } setError(""); addItem({ ...product, price: finalPrice(product), currency: product.regional_price?.currency || "EUR" }, quantity, options); setAdded(true); window.setTimeout(() => setAdded(false), 1800); }
  ...
  return <main className="min-h-screen bg-[var(--paper)] px-5 pb-28 pt-28 md:px-10 md:pt-32"><div className="mx-auto max-w-7xl">
    ...
    <div className="mt-8 grid gap-10 lg:grid-cols-[1.15fr_0.85fr] lg:gap-20"><ProductGallery product={product}/><div className="flex flex-col justify-center py-5"><div className="flex items-center justify-between gap-4"><p className="text-[9px] uppercase tracking-[0.3em] text-black/40">{product.branch} / {product.category}</p><WishlistButton productId={product.id}/></div><h1 className="mt-5 font-serif text-[clamp(3.5rem,7vw,7rem)] leading-[0.76] tracking-[-0.065em]">{product.name}</h1><p className="mt-8 max-w-lg text-sm leading-7 text-black/55">{product.description || "A piece from the EsteeHouse collection, made to carry its own character."}</p><div className="mt-8 space-y-5 border-y border-black/10 py-6">{product.material_options?.length > 1 && <label className="block text-[9px] uppercase tracking-[.18em] text-black/45">Material<select value={options.material} onChange={(e) => setOptions((v) => ({ ...v, material: e.target.value }))} className="mt-2 w-full border-b border-black/15 bg-transparent py-3 text-sm normal-case tracking-normal outline-none">{product.material_options.map((item) => <option key={item}>{item}</option>)}</select></label>}{product.material_options?.length === 1 && <p className="text-[9px] uppercase tracking-[.18em] text-black/45">Material <span className="ml-3 normal-case tracking-normal text-black">{product.material_options[0]}</span></p>}{product.size_type !== "none" && product.size_options?.length > 0 && <label className="block text-[9px] uppercase tracking-[.18em] text-black/45">{product.size_type === "ring" ? "Ring size" : product.size_type === "bracelet" ? "Bracelet size" : "Necklace length"}<select value={options.size} onChange={(e) => setOptions((v) => ({ ...v, size: e.target.value }))} className="mt-2 w-full border-b border-black/15 bg-transparent py-3 text-sm normal-case tracking-normal outline-none">{product.size_options.map((item) => <option key={item}>{item}</option>)}</select></label>}{product.stone_options?.length > 0 && <label className="block text-[9px] uppercase tracking-[.18em] text-black/45">Stone{product.stone_required ? " · required" : ""}<select value={options.stone} onChange={(e) => setOptions((v) => ({ ...v, stone: e.target.value }))} className="mt-2 w-full border-b border-black/15 bg-transparent py-3 text-sm normal-case tracking-normal outline-none"><option value="">Choose a stone</option>{product.stone_options.map((item) => { const name = typeof item === "string" ? item : item.name; return <option key={name} value={name}>{name}</option>; })}</select></label>}{product.gold_available && <p className="text-[9px] leading-5 text-black/45">Gold made-to-order is available for this piece. Final gold pricing is confirmed with the house before payment.</p>}{product.customization_note && <p className="text-xs leading-6 text-black/50">{product.customization_note}</p>}</div><div className="mt-8 border-y border-black/10 py-6">{discounted && <p className="text-sm text-black/35 line-through">{symbol}{regional.amount.toFixed(2)}</p>}<p className="text-3xl tracking-[-0.03em]">{symbol}{price.toFixed(2)}</p>{discounted && <p className="mt-2 text-[9px] uppercase tracking-[0.2em]">{product.discount_percent}% house offer</p>}<p className="mt-2 text-[9px] uppercase tracking-[0.2em] text-black/40">{regional.currency} pricing · taxes and shipping calculated at checkout</p></div><div className="mt-8 flex items-center justify-between border-b border-black/15 pb-4"><span className="text-[9px] uppercase tracking-[0.22em]">Quantity</span><div className="flex items-center gap-5"><button type="button" onClick={() => setQuantity((q) => Math.max(1, q - 1))} aria-label="Decrease quantity"><Minus size={15}/></button><span className="w-5 text-center text-sm">{quantity}</span><button type="button" onClick={() => setQuantity((q) => q + 1)} aria-label="Increase quantity"><Plus size={15}/></button></div></div><button type="button" onClick={addToCart} className="mt-8 flex w-full items-center justify-center gap-3 rounded-full bg-black px-6 py-4 text-[9px] uppercase tracking-[0.25em] text-white transition hover:translate-y-[-1px]"><ShoppingBag size={15}/> {added ? "Added to bag" : "Add to bag"}</button><Link href="/shipping" className="mt-5 text-center text-[9px] uppercase tracking-[0.2em] text-black/45 underline underline-offset-4">Shipping & returns</Link></div></div>
