...
export async function POST(request, { params }) {
  ...
  try {
    ...
    if (!token) {
      return NextResponse.redirect(new URL("/checkout/result?status=failure&reason=missing-token", request.url));
    }

    const { data: payment, error: paymentLookupError } = await supabaseAdmin
      .from("commerce_payments")
      .select("id, order_id, amount, currency, token, payment_id, payment_status")
      .eq("conversation_id", conversationId)
      .maybeSingle();

    if (paymentLookupError || !payment) {
      return NextResponse.redirect(new URL("/checkout/result?status=failure&reason=payment-record-not-found", request.url));
    }
    if (payment.token && payment.token !== token) {
      return NextResponse.redirect(new URL("/checkout/result?status=failure&reason=invalid-token", request.url));
    }
    ...
    if (orderError || !order) {
      return NextResponse.redirect(new URL("/checkout/result?status=failure&reason=order-not-found", request.url));
    }

    // Callback retries are safe: once a payment is finalized, return the existing order.
    ...
    const paymentTransactionId = transaction?.paymentTransactionId || null;
    ...
    if (finalizeError) {
      ...
      // Do not mark a verified payment as failed. The payment succeeded; this requires operational review.
      await supabaseAdmin.from("commerce_payments").update({
        payment_status: "success",
        provider_status: "success_finalization_error",
      }).eq("id", payment.id);
      return NextResponse.redirect(new URL(`/checkout/result?status=review&conversationId=${encodeURIComponent(conversationId)}`, request.url));
    }
