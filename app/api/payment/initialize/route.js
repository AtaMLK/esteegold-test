...
import { calculateLine, resolveCatalogItem } from "../../../_lib/storeCatalog";
...
function createCheckoutForm(request) {
  return new Promise((resolve, reject) => {
    client.checkoutFormInitialize.create(request, (error, result) => error ? reject(error) : resolve(result));
  });
}
...
export async function POST(request) {
  ...
  try {
    ...
    const safeItems = items.map((item) => {
      ...
      const pricing = calculateLine(catalogItem, quantity);
      return { id: catalogItem.id, catalogItem, quantity, pricing, options: item.options && typeof item.options === "object" ? item.options : {} };
    });

    const subtotal = Number(safeItems.reduce((sum, item) => sum + item.pricing.lineTotal, 0).toFixed(2));
    ...
    const newOrder = {
      ...
      total,
      customer_snapshot: {
        fullName: cleanName(customer.fullName),
        email: cleanName(customer.email),
        phone: cleanName(customer.phone, ""),
      },
      address_snapshot: {
        address: cleanName(address.address),
        city: cleanName(address.city),
        postalCode: cleanName(address.postalCode),
        country: "Turkey",
      },
      terms_accepted: true,
    ...
    const { data: order, error: orderError } = await supabaseAdmin.from("commerce_orders").insert(newOrder).select("id, order_number, total").single();
    if (orderError) throw new Error(`ORDER_CREATE_FAILED:${orderError.message}`);
    createdOrderId = order.id;

    const orderItems = safeItems.map(({ id, catalogItem, quantity, pricing, options }) => ({
      order_id: order.id,
      product_id: id,
      product_line: catalogItem.productLine,
      product_name_snapshot: catalogItem.name,
      category_snapshot: catalogItem.category,
      options_snapshot: options,
      quantity,
      unit_list_price: pricing.listPrice,
      unit_discount: pricing.unitDiscount,
      unit_final_price: pricing.unitFinalPrice,
      line_total: pricing.lineTotal,
    }));

    const { error: itemError } = await supabaseAdmin.from("commerce_order_items").insert(orderItems);
    ...
    const result = await createCheckoutForm({
      ...
      callbackUrl: `${siteUrl}/api/payment/callback/${encodeURIComponent(conversationId)}`,
      ...
      buyer: {
        ...
        identityNumber: String(customer.identityNumber || "00000000000"),
      ...
      shippingAddress: { contactName: customer.fullName, city: address.city, country: "Turkey", address: address.address, zipCode: address.postalCode },
      billingAddress: { contactName: customer.fullName, city: address.city, country: "Turkey", address: address.address, zipCode: address.postalCode },
      basketItems: safeItems.map(({ id, catalogItem, quantity, pricing }) => ({
        id: `BI-${id}`,
        name: catalogItem.name,
        category1: catalogItem.category,
        itemType: Iyzipay.BASKET_ITEM_TYPE.PHYSICAL,
        price: pricing.lineTotal.toFixed(2),
      })),
    });
    ...
    if (result.status !== "success" || !result.paymentPageUrl || !result.token) {
      ...
      throw new Error(result.errorMessage || "Unable to initialize payment.");
    }

    const { error: paymentError } = await supabaseAdmin.from("commerce_payments").insert({
      order_id: order.id,
      provider: "iyzico",
