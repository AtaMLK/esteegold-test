...
export async function POST(request) {
  ...
  try {
    ...
    if (productMap.size !== ids.length) return NextResponse.json({ error: "One or more products are no longer available." }, { status: 409 });
    ...
    if (total <= 0) return NextResponse.json({ error: "Invalid checkout total." }, { status: 400 });
