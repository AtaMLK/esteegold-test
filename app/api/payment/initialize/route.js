...
export async function POST(request) {
  ...
  try {
    ...
    const result = await createCheckoutForm({ locale: Iyzipay.LOCALE.EN, conversationId, price: total.toFixed(2), paidPrice: total.toFixed(2), currency: iyziCurrency, basketId: order.order_number, paymentGroup: Iyzipay.PAYMENT_GROUP.PRODUCT, callbackUrl: `${siteUrl}/api/payment/callback/${encodeURIComponent(conversationId)}`, enabledInstallments: [1], buyer: { id: order.order_number, name, surname, gsmNumber: cleanName(customer.phone, "+900000000000"), email: customerSnapshot.email, identityNumber: String(customer.identityNumber || "00000000000"), registrationAddress: addressSnapshot.address, city: addressSnapshot.city, country: addressSnapshot.country, zipCode: addressSnapshot.postalCode, ip: request.headers.get("x-forwarded-for")?.split(",")[0]?.trim() || "127.0.0.1" }, shippingAddress: { contactName: customerSnapshot.fullName, city: addressSnapshot.city, country: addressSnapshot.country, address: addressSnapshot.address, zipCode: addressSnapshot.postalCode }, billingAddress: { contactName: customerSnapshot.fullName, city: addressSnapshot.city, country: addressSnapshot.country, address: addressSnapshot.address, zipCode: addressSnapshot.postalCode }, basketItems: safeItems.map(({ id, product, quantity, pricing }) => ({ id: `BI-${id}`, name: product.name, category1: product.category, itemType: Iyzipay.BASKET_ITEM_TYPE.PHYSICAL, price: (pricing.finalPrice * quantity).toFixed(2) })) });
