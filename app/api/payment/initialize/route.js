...
export async function POST(request) {
  try {
    ...
    if (!customer?.email || !customer?.fullName || !customer?.identityNumber) return NextResponse.json({ error: "Name, email and identity number are required." }, { status: 400 });
    ...
    const basketItems = safeItems.map(({ id, catalogItem, quantity }) => ({ id: `BI-${id}`, name: catalogItem.name, category1: catalogItem.category, itemType: Iyzipay.BASKET_ITEM_TYPE.PHYSICAL, price: (catalogItem.price * quantity).toFixed(2) }));
    ...
    const result = await createCheckoutForm({
      ...
      buyer: { id: conversationId, name, surname, gsmNumber: cleanName(customer.phone, "+900000000000"), email: customer.email, identityNumber: String(customer.identityNumber), registrationAddress: address.address, city: address.city, country: "Turkey", zipCode: address.postalCode, ip: request.headers.get("x-forwarded-for")?.split(",")[0]?.trim() || "127.0.0.1" },
