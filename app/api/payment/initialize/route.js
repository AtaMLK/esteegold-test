...
function orderNumber() {
  return `EH-${new Date().toISOString().slice(0, 10).replaceAll("-", "")}-${crypto.randomUUID().replaceAll("-", "").slice(0, 8).toUpperCase()}`;
}
  ...
  try {
    if (!process.env.IYZIPAY_API_KEY || !process.env.IYZIPAY_SECRET_KEY)
      return NextResponse.json(
        { error: "Iyzico API credentials are not configured." },
        { status: 503 },
      );
    ...
    if (!acceptedTerms || !acceptedShipping)
      return NextResponse.json(
        { error: "Terms and shipping acceptance are required." },
        { status: 400 },
      );
    ...
    const subtotal = Number(
      safeItems
        .reduce((sum, item) => sum + item.pricing.listPrice * item.quantity, 0)
        .toFixed(2),
    );
    const discountTotal = Number(
      safeItems
        .reduce(
          (sum, item) => sum + item.pricing.unitDiscount * item.quantity,
          0,
        )
        .toFixed(2),
    );
    ...
    const customerSnapshot = {
      ...
      email: cleanName(customer.email).toLowerCase(),
    ...
    const addressSnapshot = {
      ...
      country: cleanName(address.country, "Turkey"),
    };
    ...
    const { error: itemError } = await supabaseAdmin
      ...
      .insert(
        safeItems.map(({ id, product, quantity, pricing, options }) => ({
          order_id: order.id,
          product_id: id,
          product_line: product.branch,
          ...
          unit_final_price: pricing.finalPrice,
          line_total: pricing.finalPrice * quantity,
        })),
      );
    if (itemError)
      throw new Error(`ORDER_ITEMS_CREATE_FAILED:${itemError.message}`);
    ...
    const result = await createCheckoutForm({
      ...
      buyer: {
        ...
        registrationAddress: addressSnapshot.address,
        city: addressSnapshot.city,
        country: addressSnapshot.country,
        zipCode: addressSnapshot.postalCode,
        ip:
          request.headers.get("x-forwarded-for")?.split(",")[0]?.trim() ||
          "127.0.0.1",
      },
      shippingAddress: {
        contactName: customerSnapshot.fullName,
        city: addressSnapshot.city,
        country: addressSnapshot.country,
      ...
      basketItems: safeItems.map(({ id, product, quantity, pricing }) => ({
        id: `BI-${id}`,
        name: product.name,
        category1: product.category,
        itemType: Iyzipay.BASKET_ITEM_TYPE.PHYSICAL,
        price: (pricing.finalPrice * quantity).toFixed(2),
      })),
    });
    ...
    if (
      ...
      await supabaseAdmin
        ...
        .eq("id", order.id);
      return NextResponse.json(
        { error: result.errorMessage || "Unable to initialize payment." },
        { status: 502 },
      );
    }
