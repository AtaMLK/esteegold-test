...
export async function GET(request) {
  try {
    ...
    const { data, error } = await adminClient().from("commerce_products").select("id,name,branch,category,description,image_url,price,discount_percent,active,created_at,updated_at").order("created_at", { ascending: false });
    if (error) throw error;
...
function clean(body, { requireStock = false } = {}) {
  ...
  return { row: { name: body.name.trim(), branch: body.branch, category: body.category?.trim() || "Uncategorized", description: body.description?.trim() || "", image_url: body.image_url?.trim() || null, price, discount_percent: discount, active: body.active !== false, updated_at: new Date().toISOString() }, stock };
}
...
export async function PATCH(request) {
  try {
    ...
    if (Object.keys(body).every((key) => ["id", "active"].includes(key))) {
      ...
      if (error) throw error;
      const [product] = await withStock([data]);
