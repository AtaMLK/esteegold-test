...
import { ArrowLeft, ArrowUpRight, Check, CreditCard, ExternalLink } from "lucide-react";
    ...
    <main className="min-h-screen bg-[#f3f0e9] text-[#171714] px-[6vw] py-28">
      ...
      <div className="grid gap-14 lg:grid-cols-[1fr_360px] mt-10">
        <div className="max-w-2xl">
          {!readyForPayment ? (
            <form onSubmit={(event) => { event.preventDefault(); setReadyForPayment(true); }}>
            ...
            <section>
              <p className="text-[9px] uppercase tracking-[.22em] text-black/45">02 / PAYMENT</p>
              <h1 className="mt-3 font-serif text-6xl leading-[.85]">Choose how<br />to pay.</h1>
        ...
        <aside className="h-fit bg-[#191915] text-[#f3f0e9] p-7 lg:sticky lg:top-24">
          ...
          <div className="mt-6 flex items-start gap-3 text-[9px] leading-5 text-white/40"><CreditCard size={15} /> Card details are handled directly by the selected payment provider.</div>
        </aside>
      </div>
    </main>
  );
