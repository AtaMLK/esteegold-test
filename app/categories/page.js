...
export default function CategoriesPage() {
  ...
  function submitSearch(event) {
    ...
    window.history.replaceState({}, "", `/categories${params.toString() ? `?${params.toString()}` : ""}`);
    setCategory("all");
  ...
  function clearSearch() {
    ...
    window.history.replaceState({}, "", `/categories${params}`);
  }
    ...
    <main className="min-h-screen bg-[var(--paper)] px-5 pb-24 pt-32 md:px-10">
      <div className="mx-auto max-w-7xl">
        <section className="grid gap-10 border-b border-black/10 pb-12 md:grid-cols-[1fr_0.7fr] md:items-end">
          ...
          <p className="max-w-md text-sm leading-7 text-black/50">I make pieces to be worn, carried and lived with. Start with a collection, search by name or material, and take your time.</p>
        </section>

        <section className="flex flex-col gap-5 border-b border-black/10 py-5">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex flex-wrap gap-2">
              <Link href={`/categories${search ? `?search=${encodeURIComponent(search)}` : ""}`} className={`rounded-full border px-4 py-2 text-[8px] uppercase tracking-[0.2em] ${branch === "all" ? "bg-black text-white" : "border-black/15"}`}>All</Link>
              <Link href={`/categories?branch=gold${search ? `&search=${encodeURIComponent(search)}` : ""}`} className={`rounded-full border px-4 py-2 text-[8px] uppercase tracking-[0.2em] ${branch === "gold" ? "bg-black text-white" : "border-black/15"}`}>EsteeGold</Link>
              <Link href={`/categories?branch=bags${search ? `&search=${encodeURIComponent(search)}` : ""}`} className={`rounded-full border px-4 py-2 text-[8px] uppercase tracking-[0.2em] ${branch === "bags" ? "bg-black text-white" : "border-black/15"}`}>EsteeBags</Link>
            </div>
          ...
          <form onSubmit={submitSearch} className="flex items-center gap-3 border-b border-black/20 py-2">
            <Search size={16} aria-hidden="true" />
            <input value={searchInput} onChange={(event) => setSearchInput(event.target.value)} placeholder="Search pieces, materials or collections" aria-label="Search products" className="min-w-0 flex-1 bg-transparent py-2 text-sm outline-none" />
            {searchInput && <button type="button" onClick={clearSearch} aria-label="Clear search" className="p-2 text-black/45 hover:text-black"><X size={15} /></button>}
            <button type="submit" className="rounded-full bg-black px-4 py-2 text-[8px] uppercase tracking-[0.2em] text-white">Search</button>
          </form>
        </section>
